...
public class Libro implements Serializable {
    ...
    public static void cargarLibro() {
        String filePath = "src\\resources\\Biblioteca.dat";
    ...
    public static void escribirLibro() {
        try (FileOutputStream fos = new FileOutputStream("src\\resources\\Biblioteca.dat");
             ObjectOutputStream oos = new ObjectOutputStream(fos)) {
            oos.writeObject(libros);
            for (int i = 0; i < libros.size(); i++) {
                Libro libro = libros.get(i);
                System.out.println((i + 1) + ". ISBN: " + libro.getIsbn() + " | Título: " + libro.getTitulo() + " | Autor: " + libro.getAutor() + " | Fecha de publicación: " + libro.getFechaPublicacion());
            }
    ...
    public static void crearLibro() {
        ...
        String isbn = generarYvalidarISBN();
    ...
    public static String generarYvalidarISBN() {
        ...
        int checksum = calcularDigitoFalla(isbnBuilder.toString());
        isbnBuilder.append(checksum);

        String isbn = isbnBuilder.toString();

        // Validar el ISBN generado
        if (isbnValido(isbn) && !isbnExistente(isbn)) {
                  return isbn; // Retornar el ISBN si es válido 
                } else { 
                   return "ISBN existente." + "Creando uno nuevo" +  generarYvalidarISBN(); 
                }
            }
         
        
            private static int calcularDigitoFalla(String isbn) {
        int total = 0;
        for (int i = 0; i < 12; i++) {
...
private static boolean isbnExistente(String isbn) {
