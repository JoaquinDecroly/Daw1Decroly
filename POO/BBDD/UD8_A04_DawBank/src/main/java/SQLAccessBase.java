...
public class SQLAccessBase {
    ...
    static void eliminarCuentaBancaria(String iban) {
        String sql = "DELETE FROM CUENTABANCARIA WHERE IBAN = ?";
        ...
        try (Connection connection = SQLManager.getConecction();
            ...
            if (filasAfectadas > 0) {
                ...
                System.out.println("No se encontró una cuenta con el IBAN proporcionado.");
            }
        } catch (SQLException e) {
            System.err.println("Error al eliminar la cuenta bancaria: " + e.getMessage());
        }
    }
    
        static List<CuentaBancaria> getCuentasBancarias() {
            List<CuentaBancaria> cuentas = new LinkedList<>();
            String sql = "SELECT * FROM VISTA_SALDO";
